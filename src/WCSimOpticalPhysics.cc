...
// Modified:    Mohir FffGola 10th July 2023
//----------------------------------------------------------------------------
//
...
G4ThreadLocal G4Scintillation *WCSimOpticalPhysics::fScintillationProcess = NULL;
G4ThreadLocal G4Cerenkov *WCSimOpticalPhysics::fCerenkovProcess = NULL;
G4ThreadLocal G4OpWLS *WCSimOpticalPhysics::fWLSProcess = NULL;
G4ThreadLocal G4OpWLS2 *WCSimOpticalPhysics::fWLS2Process = NULL;
G4ThreadLocal G4OpAbsorption *WCSimOpticalPhysics::fAbsorptionProcess = NULL;
G4ThreadLocal G4OpRayleigh *WCSimOpticalPhysics::fRayleighProcess = NULL;
G4ThreadLocal G4OpMieHG *WCSimOpticalPhysics::fMieProcess = NULL;
G4ThreadLocal WCSimOpBoundaryProcess *WCSimOpticalPhysics::fBoundaryProcess = NULL; // custom boundary process
...
void WCSimOpticalPhysics::ConstructProcess()
{
  ...
  // WHY WOULD YOU DO THIS????? THIS IS SO FUCKING DUMB
  for (G4int i = 0; i < kNoProcess; i++)
...
#else
  G4ParticleTable::G4PTblDicIterator *myParticleIterator = aParticleIterator;
#endif
  ...
  for (G4int i = 0; i < kNoProcess; i++)
  {
    if (fProcessUse[i])
    {
      if (OpProcesses[i] != NULL)
      {
        OpProcesses[i]->SetVerboseLevel(verboseLevel);
      }
      else
      {
        std::cout << "Optical Process is still null! Maybe " << static_cast<G4OpticalProcessIndex>(i) << std::endl;
      }
    }
  }
...
{
  // This should be set using a material property tag, not like this
  std::runtime_error("Invalid way of setting the scintillation yield!");
}

void WCSimOpticalPhysics::SetScintillationExcitationRatio(G4double val)
{
  // This should be set using a material property tag (YIELDRATIO), not like this
  std::runtime_error("Invalid way of setting the scintillation excitation ration!");
}
