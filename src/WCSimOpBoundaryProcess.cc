// Modified from G4OpBoundaryProcess.cc of geant4.10 to implement photocathode
// physics, which is a thin film of semiconductor alloy coated on glass Model
// the reflection/transmission/absorption processes by the coated layer
//
// CoatedDielectricDielectric_Model1() :
// Copy from geant4.11, which is based on
// https://ieeexplore.ieee.org/document/9875513 Model the alloy as a thin layer
// with real refractive index, then calculate reflection and transmission
// probability Cannot handle total internal reflection when n1<n2
//
// CoatedDielectricDielectric_Model2() :
// Implementation based on https://arxiv.org/abs/physics/0408075v1
// Model the alloy as a thin layer with real and imaginary refractive indices,
// then calculate absorption, reflection and transmission probability
//
// ********************************************************************
...
{
    if (what == Absorption || what == Detection)
    {
        return BAbsorption;
    }
    ...
    else if (what == FresnelReflection || what == TotalInternalReflection || what == LambertianReflection || what == LobeReflection || what == SpikeReflection || what == CoatedDielectricReflection || what == BackScattering)
    {
        return BReflection;
    }
    else if (static_cast<int>(what) >= static_cast<int>(PolishedLumirrorAirReflection) && static_cast<int>(what) <= static_cast<int>(CoatedDielectricReflection))
    {
        return BReflection;
    }
...
// WCSimOpBoundaryProcess::WCSimOpBoundaryProcess(const WCSimOpBoundaryProcess
// &right)
// {
// }
                                                        ...
                                                        const G4Step &aStep)
{
    ...
    else
    {
        ...
        G4Exception(
            "WCSimOpBoundaryProcess::PostStepDoIt", "OpBoun01", EventMustBeAborted,
            ed,
    ...
    else
    {
        ...
        std::cout << "Killed for bad sad reason" << std::endl;
        return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
    }
    ...
    if (OpticalSurface)
    {
        ...
        if (aMaterialPropertiesTable)
        {
            ...
            if (aMaterialPropertiesTable->ConstPropertyExists("SURFACEROUGHNESS"))
                theSurfaceRoughness =
                    aMaterialPropertiesTable->GetConstProperty("SURFACEROUGHNESS");
    ...
    else if (static_cast<int>(type) == 101) // brute-force index to get coated surface processes
    {
        CoatedDielectricDielectric_Model1();
    }
    else if (static_cast<int>(type) == 102)
    {
        CoatedDielectricDielectric_Model2();
    }
    else
...
void WCSimOpBoundaryProcess::DielectricLUT()
{
    ...
    theStatus = WCSimOpBoundaryProcessStatus(
...
void WCSimOpBoundaryProcess::DielectricDichroic()
{
    ...
    if (DichroicVector)
    {
        ...
        theTransmittance =
            DichroicVector->Value(wavelength / nm, angleIncident, idx, idy) *
            perCent;
        //            G4cout << "wavelength: " << std::floor(wavelength/nm)
        //                                     << "nm" << G4endl;
        //            G4cout << "Incident angle: " << angleIncident << "deg" <<
        //            G4endl; G4cout << "Transmittance: "
        //                   << std::floor(theTransmittance/perCent) << "%" <<
        //                   G4endl;
    }
    else
    {
...
// ---------------
//
G4double WCSimOpBoundaryProcess::GetMeanFreePath(const G4Track &, G4double,
...
{
    G4MaterialPropertyVector *pp = NULL;
    ...
    do
    {
        ...
        transCoeff = 1. - GetReflectivityThroughThinLayer(sintTL, E1_perp, E1_parl,
                                                          wavelength, cost1, cost2);
        if (!G4BooleanRand(transCoeff))
        {
            if (verboseLevel > 2)
            ...
            { // incident ray oblique
                ...
                if (sint2 * sint2 > 1.)
                    NewPolarization =
                        -OldPolarization +
                        (2. * OldPolarization * theFacetNormal) * theFacetNormal;
            }
            else
            { // incident ray perpendicular
                if (Rindex2 > Rindex1)
...
G4double WCSimOpBoundaryProcess::GetReflectivityThroughThinLayer(
    G4double sinTL, G4double E1_perp, G4double E1_parl, G4double wavelength,
    G4double costh1, G4double costh2)
{
    ...
    if (sinTL >= 1.0)
    {
        ...
        { // Frustrated transmission
            ...
            // TE
            r1toTL =
                (Rindex1 * costh1 - i * gammaTL) / (Rindex1 * costh1 + i * gammaTL);
            rTLto2 =
                (i * gammaTL - Rindex2 * costh2) / (i * gammaTL + Rindex2 * costh2);
            if (costh1 != 0.0)
            {
                rTE =
                    (r1toTL + rTLto2 * std::exp(-2 * k0 * fCoatedThickness * gammaTL)) /
                    (1.0 +
                     r1toTL * rTLto2 * std::exp(-2 * k0 * fCoatedThickness * gammaTL));
            }
            // TM
    ...
    else // if (sinTL < 1.0)
    {
        ...
        // TE
        r1toTL = (Rindex1 * costh1 - fCoatedRindex * costTL) /
                 (Rindex1 * costh1 + fCoatedRindex * costTL);
        rTLto2 = (fCoatedRindex * costTL - Rindex2 * costh2) /
                 (fCoatedRindex * costTL + Rindex2 * costh2);
        if (costh1 != 0.0)
        {
            rTE = (r1toTL + rTLto2 * std::exp(2.0 * i * k0 * fCoatedRindex *
                                              fCoatedThickness * costTL)) /
                  (1.0 + r1toTL * rTLto2 *
                             std::exp(2.0 * i * k0 * fCoatedRindex *
                                      fCoatedThickness * costTL));
        }
        // TM
        r1toTL = (Rindex1 * costTL - fCoatedRindex * costh1) /
                 (Rindex1 * costTL + fCoatedRindex * costh1);
        rTLto2 = (fCoatedRindex * costh2 - Rindex2 * costTL) /
                 (fCoatedRindex * costh2 + Rindex2 * costTL);
        if (costh1 != 0.0)
        {
            rTM = (r1toTL + rTLto2 * std::exp(2.0 * i * k0 * fCoatedRindex *
                                              fCoatedThickness * costTL)) /
                  (1.0 + r1toTL * rTLto2 *
                             std::exp(2.0 * i * k0 * fCoatedRindex *
                                      fCoatedThickness * costTL));
...
void WCSimOpBoundaryProcess::CoatedDielectricDielectric_Model2()
{
    ...
    do
    {
        ...
        // Calculate reflection and transmission probability from Eq. 2 of
        // https://arxiv.org/abs/physics/0408075v1 Rename the variables to match
        // those in paper
        costh2 = cost1;
        ...
        rp = r23p + (t23p * t32p * r34p * exp(2. * i * delta)) /
                        (1. + r23p * r34p * exp(2. * i * delta));
        tp =
            t23p * t34p * exp(i * delta) / (1. + r23p * r34p * exp(2. * i * delta));
        Rp = real(rp * conj(rp));
        Tp = real(n4 * costh4 / n2 / costh2 * tp * conj(tp));

        rs = r23s + (t23s * t32s * r34s * exp(2. * i * delta)) /
                        (1. + r23s * r34s * exp(2. * i * delta));
        ts =
            t23s * t34s * exp(i * delta) / (1. + r23s * r34s * exp(2. * i * delta));
        Rs = real(rs * conj(rs));
        Ts = real(n4 * costh4 / n2 / costh2 * ts * conj(ts));

        Rtot = (Rs * E1_perp * E1_perp + Rp * E1_parl * E1_parl) /
               (E1_perp * E1_perp + E1_parl * E1_parl);
        Ttot = (Ts * E1_perp * E1_perp + Tp * E1_parl * E1_parl) /
               (E1_perp * E1_perp + E1_parl * E1_parl);
        ...
        if (rdm < Rtot)
        {
            ...
            { // incident ray oblique
                ...
                if (std::arg(rp) < -pi / 2 || std::arg(rp) > pi / 2)
                    E2_parl *= -1;
                E2_perp = sqrt(Rs) * E1_perp;
                if (std::arg(rs) < -pi / 2 || std::arg(rs) > pi / 2)
                    E2_perp *= -1;
        ...
        { // photon gets transmitted
            ...
            { // incident ray oblique
                ...
                if (std::arg(tp) < -pi / 2 || std::arg(tp) > pi / 2)
                    E2_parl *= -1;
                if (std::arg(ts) < -pi / 2 || std::arg(ts) > pi / 2)
                    E2_perp *= -1;
                E2_total = E2_perp * E2_perp + E2_parl * E2_parl;
                E2_abs = std::sqrt(E2_total);
                C_parl = E2_parl / E2_abs;
